/**
 * @file <argos3/plugins/robots/pi-puck/simulator/pipuck_light_default_sensor.cpp>
 *
...
#include "pipuck_light_default_sensor.h"
...
namespace argos {
   ...
   CPiPuckLightDefaultSensor::CPiPuckLightDefaultSensor() :
      m_bEnabled(true),
   ...
   void CPiPuckLightDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the pipuck light default sensor", ex);
      }
   }
   ...
   void CPiPuckLightDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CPiPuckLightDefaultSensor::Update() {
      if (!m_bEnabled) {
        return;
      }
      /* Erase readings */
   ...
   void CPiPuckLightDefaultSensor::Reset() {
   ...
   void CPiPuckLightDefaultSensor::Enable() {
     m_bEnabled = true;
   }
   ...
   void CPiPuckLightDefaultSensor::Disable() {
     m_bEnabled = false;
   }
   ...
   REGISTER_SENSOR(CPiPuckLightDefaultSensor,
                   "pipuck_light", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The pipuck light sensor (optimized for 2D).",
